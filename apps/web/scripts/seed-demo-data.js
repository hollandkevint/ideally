/**
 * Demo Environment Seeding Script
 * Populates database with compelling demo scenarios for video creation
 */
...
async function createDemoUser() {
  ...
  console.log('Demo user created:', authData?.user?.id || 'already exists')
  return authData?.user
}

async function seedDemoWorkspaces(userId) {
  ...
  for (const scenario of demoScenarios) {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        chat_context: scenario.chat_context,
        canvas_elements: []
      })
      .select()
      .single()
    ...
    if (error) {
      console.error(`Error creating workspace "${scenario.name}":`, error)
    } else {
      console.log(`✅ Created workspace: "${scenario.name}" (ID: ${data.id})`)
    }
  }
}
...
async function main() {
  try {
    ...
    // Try to find existing demo user by email first
    ...
    // Clear existing demo workspaces
    ...
    if (deleteError) {
      console.warn('Error clearing existing workspaces:', deleteError)
    }
    
    // Seed new demo workspaces
    await seedDemoWorkspaces(userId)
